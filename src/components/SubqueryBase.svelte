<script>
    ...
    let queryBase = "Person";
    let selectedBaseId = ALL_DATA["Person"]?.[0]?.id ?? null;
    ...
    $: personIdFilterValue = (() => {
        const personIdFilter = activeFilters.find(
            (f) =>
                f.active &&
                (f.key === "person.id" || f.key.endsWith(".person.id")),
        );
        return personIdFilter ? personIdFilter.value : null;
    })();
    ...
    $: isSubqueryLabelValid = subqueryModalConfig.label.trim().length > 0;
    ...
    $: availableDataPalette = (() => {
        ...
        try {
            BASE_FIELDS[queryBase].forEach((field) => {
                const key = `${queryBase.toLowerCase()}.${field}`;
                const isFilterable = key === "person.id";
            ...
            for (const tableName in activeJoins) {
                ...
                currentJoins.forEach((join) => {
                    ...
                    BASE_FIELDS[tableName].forEach((baseField) => {
                        const key = `${tableName}.${join.key}.${baseField}`;
                        const isFilterable =
                            key.toLowerCase() === "person.id" ||
                            (tableName === "Person" && baseField === "id");
                        if (!uniqueKeys.has(key)) {
                            paletteItems.push({
                                key,
                                origin: tableName,
    ...
    $: resolvedPreviewData = (() => {
        ...
        try {
            ...
            if (personIdFilterValue !== null) {
                const trimmedFilter = (personIdFilterValue ?? "")
                    .toString()
                    .trim();

                if (trimmedFilter !== "") {
                    const isValidInteger = /^\d+$/.test(trimmedFilter);

                    if (isValidInteger) {
                        filterId = parseInt(trimmedFilter, 10);

                        if (!isNaN(filterId)) {
                            if (queryBase === "Person") {
                                filteredBaseData = baseData.filter(
                                    (r) => r?.id === filterId,
                                );
                            } else {
                                filteredBaseData = baseData.filter(
    ...
    const updatePaths = () => {
        ...
        // All subquery path/bracket logic has been removed
        // We just clear the brackets array if it has anything
    ...
    function applySubqueryExport() {
        const { base, outputType, label, dropIndex } = subqueryModalConfig;
        ...
        const newExport = {
            ...
            filters: [], // No filters in this version
            color: newColor,
        };
        ...
        >
                    ...
                    <div class="custom-join-group">
                        <div>
                            ...
                            <input
                                type="text"
                                id="subquery-label"
                                placeholder="e.g., school_count or address_count"
                                bind:value={subqueryModalConfig.label}
                                maxlength="30"
                            />
                        </div>
                        <div>
                            <label for="subquery-base">Subquery Base</label>
                            <select
                                id="subquery-base"
                                bind:value={subqueryModalConfig.base}
                            >
            ...
            <div class="modal-actions">
                ...
                <button
                    ...
                    disabled={!isSubqueryLabelValid}>Add Export</button
                >
            </div>
        </div>
    </div>
...
{#if isPreviewModalOpen}
    ...
    >
        ...
        >
            ...
            {#if personIdFilterValue !== null && personIdFilterValue
                    .toString()
                    .trim() !== ""}
                {@const displayId = personIdFilterValue.toString().trim()}
                {@const parsedId = parseInt(displayId, 10)}
                {#if !isNaN(parsedId)}
                    <div class="filter-status-bar">
                        Filtering for <strong>Person ID = {parsedId}</strong>.
                        Showing
                ...
                {:else if displayId !== ""}
                    <div class="filter-status-bar error">
                        Invalid Person ID entered. Showing 0 records.
                    </div>
                {/if}
            {/if}
                        ...
                        <tbody>
                            {#if resolvedPreviewData.rows.length === 0}
                                ...
                                <tr>
                                    ...
                                    >
                                        {#if filterValue !== "" && isNaN(parseInt(filterValue, 10))}
            ...
            <div class="controls-bar">
                ...
                <div class="segmented-control">
                    {#if ALL_DATA[queryBase] && ALL_DATA[queryBase].length > 0}
                        {#each ALL_DATA[queryBase].slice(0, 5) as record (record.id)}
                            <button
                    ...
                    >
                        {#if activeFilters.length === 0 && (dropIndicatorIndex === null || !isFilterZoneActive)}
                            <p class="filter-placeholder">
                                Drag a 'person.id' field here to filter by
                                Person ID.
                            </p>
                        {/if}
                                    ...
                                    <div class="palette-key">
                                        ...
                                        {#if item.isFilterable}
                                            <svg
                                                ...
                                                title="Drag to 'Filters' to filter by Person ID"
                                                viewBox="0 0 20 20"
                                                fill="currentColor"
                                                aria-hidden="true"
                                            >
...
<style>
    ...
    /* Removed subquery-bracket-path and subquery-pointer-path */
    .subquery-count-list {
