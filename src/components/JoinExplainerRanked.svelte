<script>
    ...
    $: joinedRecords = {
        School: personSchools.filter((_, i) => activeJoins.School.includes(i)),
        Address: personAddresses.filter((_, i) =>
            activeJoins.Address.includes(i),
        ),
    };
    ...
    $: previewItems = (() => {
        ...
        activeJoins.School.forEach((index) => {
            const record = personSchools[index];
            if (!record) return;
            for (const [key, value] of Object.entries(record)) {
                if (key !== "id" && key !== "personId") {
                    items.push({
                        key: `school_${index + 1}.${key}`,
                        value,
                        origin: "School",
                    });
                }
        ...
        activeJoins.Address.forEach((index) => {
            const record = personAddresses[index];
            if (!record) return;
            for (const [key, value] of Object.entries(record)) {
                if (key !== "id" && key !== "personId") {
                    items.push({
                        key: `address_${index + 1}.${key}`,
                        value,
                        origin: "Address",
                    });
                }
    ...
    $: {
        const updatePaths = async () => {
            ...
            activeJoins.Address.forEach((index) => {
                if (addressRowRefs[index]) {
                    const path = getConnectorPath(
                        personCardEl,
                        addressRowRefs[index],
                        COLOR_MAP.Address,
                    );
                    if (path) newPaths.push(path);
                }
    ...
    // --- HANDLERS ---
    function toggleJoin(tableName, index) {
        const currentJoins = activeJoins[tableName];
        const isJoined = currentJoins.includes(index);

        let newJoins;
        if (isJoined) {
            newJoins = currentJoins.filter((i) => i !== index);
        } else {
            newJoins = [...currentJoins, index];
        }
                ...
                <table class="results-table">
                    ...
                    <tbody>
                        {#each personSchools as school, i (school.id)}
                            <tr
                                class:highlighted={activeJoins.School.includes(
                                    i,
                                )}
                            ...
                            >
                                ...
                                <td>
                                    <button
                                        class="btn join-btn"
                                        class:active={activeJoins.School.includes(
                                            i,
                                        )}
                                        on:click={() => toggleJoin("School", i)}
                                    >
                                        {activeJoins.School.includes(i)
                                            ? "Joined"
                                            : "Join"} Rank {i + 1}
                                    </button>
                                </td>
                ...
                <table class="results-table">
                    ...
                    <tbody>
                        {#each personAddresses as address, i (address.id)}
                            <tr
                                class:highlighted={activeJoins.Address.includes(
                                    i,
                                )}
                            ...
                            >
                                ...
                                <td>
                                    <button
                                        class="btn join-btn"
                                        class:active={activeJoins.Address.includes(
                                            i,
                                        )}
                                        on:click={() =>
                                            toggleJoin("Address", i)}
                                    >
                                        {activeJoins.Address.includes(i)
                                            ? "Joined"
                                            : "Join"} Rank {i + 1}
