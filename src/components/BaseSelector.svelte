<script>
    ...
    function removeJoin(tableName, joinIdToRemove) {
        ...
        if (dependentExports.length > 0) {
            warningModalMessage = `Cannot remove join "${joinToRemove.key}". Please remove the following dependent exports first:\n\n${dependentExports.map((ex) => `- ${ex.key}`).join("\n")}`;
                ...
                {#if modalConfig.isParentJoin}
                    <div class="option-panel" in:fade={{ duration: 150 }}>
                        <p style="margin: 0; line-height: 1.6;">
                            This will join the single related <strong
                                >{modalConfig.tableName}</strong
                            >
            ...
            {#if personIdFilterValue !== null && personIdFilterValue
                ...
                {#if !isNaN(parsedId)}
                    <div class="filter-status-bar">
                        ...
                        Showing {resolvedPreviewData.rows.length}
            ...
            <div class="controls-bar">
                ...
                <div class="segmented-control">
                    {#if ALL_DATA[queryBase] && ALL_DATA[queryBase].length > 0}
                        {#each ALL_DATA[queryBase].slice(0, 2) as record (record.id)}
                            <button
